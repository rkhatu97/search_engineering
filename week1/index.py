...
def main(source_dir: str, file_glob: str, index_name: str, workers: int, host: str, max_docs: int, batch_size: int, refresh_interval: str):
    ...
    requests.put(f"https://{host}:9200/{index_name}/_settings", json={"index":{"refresh_interval" : refresh_interval}}, verify=False)
    ...
    requests.put(f"https://{host}:9200/{index_name}/_settings", json={"index":{"refresh_interval" : "5s"}}, verify=False)
    logger.debug(client.indices.get_settings(index=index_name))
